...
export default function RecipeCardPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh', paddingBottom: 120 }}>
      <div style={{ height: 200, background: 'var(--cream)', overflow: 'hidden', position: 'relative' }}>
        {thumbnail ? <img src={thumbnail} alt={recipe.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div className="header-art" style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 64 }}>🍽️</div>}
        <Link href="/" style={{ position: 'absolute', top: 16, left: 16, width: 48, height: 48, borderRadius: 999, background: 'var(--yellow)', border: '2px solid var(--black)', borderBottomWidth: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', fontSize: 20, fontWeight: 700, color: 'var(--black)' }}>←</Link>
      </div>
